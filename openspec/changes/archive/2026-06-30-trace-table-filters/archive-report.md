...
- **Ring buffer replaced with append-only slice**: The fixed-size ring buffer (cap 200) caused error traces to be silently overwritten by successful requests, creating a UX contradiction where `errors_total` showed errors but the trace table showed none. Replaced with an append-only slice in both backends — all request traces are preserved for the lifetime of the process. Simplifies the code (~20 lines removed) with no API changes. Acceptable for a demo application; a production system would add sampling or retention limits. See `go-service/internal/interfaces/metrics_handler.go` and `node-service/src/interfaces/metrics-handler.js`.
