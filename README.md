...
> 15 development phases completed. See [docs/phases.md](docs/phases.md) for the full history.
...
| **JWT Bearer token authentication** | Stateless auth: HMAC-SHA256 signed tokens on all `/devices` endpoints (GET, POST, PUT, DELETE). Token validated via middleware wrapping the HTTP handler chain. Credentials via env vars (demo scope) |
...
| **Append-only slice for request tracing** | In-memory append-only slice per backend — all traces preserved for process lifetime (no cap, no eviction). `sync.Mutex` (Go) / shared-nothing (Node) for thread safety. Newest-first retrieval. No external persistence — purely additive, no migration risk |
...
All business logic is developed test-first (red → green → refactor). Test files are co-located with source at every layer: domain, application, interfaces, and infrastructure. Go uses table-driven tests with `t.Run`; Node.js uses `node:test` with `describe`/`it` and the native `mock` module.

## Tech Stack

| Component | Technology |
|-----------|------------|
| Frontend | React 19, Vite 6, TypeScript 5.8, TanStack Router, TanStack Query, Tailwind CSS 4, Zod |
...
# Login and get a JWT token (all /devices endpoints require JWT)
...
# List all devices (authenticated)
...
> **Total: 703 tests** (144 Go + 85 Node.js + 430 Vitest + 44 Playwright E2E)
